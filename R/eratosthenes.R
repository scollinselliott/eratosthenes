...
# Omit: need/use of traceplot questionable for deposition/production, as they are composed of pooled Gibbs samples
# #' Traceplot of Gibbs Samples (Artifact Type Production, Use, Deposition)
# #' 
# #' Wrapper around \code{\link[graphics]{plot}} to make a traceplot of Gibbs samples from \code{\link[eratosthenes]{gibbs_ad_use}]}. See \code{\link[eratosthenes]{hist.use_marginals}} for plotting a histogram of an artifact type production, use, and deposition.
# #' 
# #' Also see \code{\link[eratosthenes]{tidy_marginals}} for exporting the results of these functions into tidy data frame for custom plotting in e.g., \code{ggplot2}.
# #' 
# #' @param marginalized A \code{list} object of class \code{marginals} or \code{use_marginals}, the output of \code{\link[eratosthenes]{gibbs_ad}} or \code{\link[eratosthenes]{gibbs_ad_use}} respectively.
# #' @param aspect A vector of one or more of \code{c("production", "use", "deposition")}. The default is all three.
# #' @param display_name The name of the artifact type to display in the histogram legend. Default the type name from the input \code{marginalized}.
# #' @param xlim The limits of the x-axis (optional).
# #' @param ylim The limits of the y-axis (optional).
# #' @param xlab Label for the y-axis. Default is \code{"Index"}.
# #' @param ylab Label for the y-axis. Default is \code{"Year"}.
...
# #' result_use <- gibbs_ad_use(result, artifacts, type = "type1")
# #' 
# #' plot(result_use, opacity = 0.5)
# #' plot(result_use, aspect = "production")
# #' 
# #' @returns A traceplot of the Gibbs samples of the selected events.
# #' 
# #' @export
# plot.use_marginals <- function(marginalized, aspect = c("production", "use", "deposition"), display_name = NULL, xlim = NULL, ylim = NULL, xlab = "Index", ylab = "Year", palette = NULL, opacity = 1, legend_pos = "topright") {
...
#         display_name <- marginalized$use$use_name
#     }
...
#     if (length(aspect) == 1) {
#         y <- dat$year
#         graphics::plot(y, type = "l", col = palette[1], xlim = xlim, ylim = ylim, xlab = xlab, ylab = ylab, main = "")
#     } else {
#         y <- dat$year
#         graphics::plot(y, type = "l", col = palette[1], xlim = xlim, ylim = ylim, xlab = xlab, ylab = ylab, main = "")
#         for (k in 2:length(aspect_name)) {
#             y <- dat[dat$aspect == aspect_name[k], ]$year
#             graphics::lines(y, type = "l", col = palette[1])
#         }
#     }
