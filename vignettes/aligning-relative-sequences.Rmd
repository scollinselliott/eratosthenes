...
The `synth_rank()` function will use recursion in order to produce a single, "merged" or "synthesized" sequence from two or more sequences.  This is accomplished by counting the total number of elements after running a recursive trace through all partial sequences (via the `quae_postea()` function, on which see below). If partial sequences are inconsistent in their rankings, a \code{NULL} value is returned.
